...
# 1. Transform behavour from old reset to new reset
# 2. Move behavior from arrows to sliders
...
from PyQt5.QtWidgets import QFileDialog, QApplication,\
  QAction, QCommonStyle, QStyle, QSplitter, QVBoxLayout,\
  QSpacerItem, QSizePolicy, QGroupBox, QCheckBox, QPushButton,\
  QHBoxLayout, QToolButton, QToolTip

from vtkUtils import hexCol, renderLinesAsTubes, AxesToTransform

ui_file = os.path.join(os.path.dirname(__file__), 'SmartLockAngles.ui')
...
class SmartLock(QMainWindow, ui):
  ...
  def onOrientationClicked(self):
    ...
    widget = self.viewUS[1] # TODO: Check this
    ...
    if sender in [self.btnRotX, self.btnRotY, self.btnRotZ]:
      ...
      if local:
        self.alignment.PostMultiply()
        self.alignment.Translate(-cursorPosition[0],
                                 -cursorPosition[1],
                                 -cursorPosition[2])
        self.alignment.RotateWXYZ(da, rotAxis)
        self.alignment.Translate(cursorPosition[0],
                                 cursorPosition[1],
                                 cursorPosition[2])
        self.alignment.PreMultiply()
      else:
        self.alignment.RotateWXYZ(da,
                                  rotAxis[0],
                                  rotAxis[1],
                                  rotAxis[2])
      ...
      if local:
        ...
        self.alignment.Translate(axis[0], axis[1], axis[2])
      else:
        self.alignment.Translate(axis[0]*dxyz,
                                          axis[1]*dxyz,
                                          axis[2]*dxyz)

    #self.alignment.Update() # Edited
    # TODO: Update contours in US
    for i in range(2):
      self.viewUS[i].UpdateContours()
    self.Render()
  ...
  def onSegClicked(self):
    ...
    ######################################
    # Write misalignment to output (error)
    ######################################

    displacementError = self.alignment.GetPosition()

    # Screen rotation - does not handle rotation
  ...
  def updateRegistration(self, rmse, icp, newContours):
    ...
    # misMatrix should be transposed
