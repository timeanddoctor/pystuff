...
data = {0 : {'colors' : [(255.0/4, 0.0, 0.0), (255.0/255.0, 0.0, 0.0)],
             'letter' : 'S'},
        1 : {'colors' : [(0.0, 255.0/4, 0.0), (0.0, 255.0/255.0, 0.0)],
             'letter' : 'C'},
        2 : {'colors' : [(0.0, 0.0, 255.0/4), (0.0, 0.0, 255.0/255.0)],
             'letter' : 'A'}}
dpi = 200
arr = 0

for i in range(3):
  for j in range(len(data[i]['colors'])):
    freeType = vtk.vtkFreeTypeTools.GetInstance()
    textProperty = vtk.vtkTextProperty()
    textProperty.SetColor(data[i]['colors'][j])
    ...
    writer.SetFileName(data[i]['letter']+ "%02d.png" %(j))
